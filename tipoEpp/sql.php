...
class Sql extends DB{
    ...
    function listarHerramientas(){
        $query = $this->connect()->prepare("select * from tipo_epp where estado = 'activo'");
        if($query->execute()){
    ...
    function agregar($item){
        $query = $this->connect()->prepare("insert tipo_epp(descripcion,estado) values(:descripcion,'activo')");
        $query->bindParam(":descripcion", $item['descripcion'], PDO::PARAM_STR);
        if($query->execute()){
            return "ok";
          }else{
